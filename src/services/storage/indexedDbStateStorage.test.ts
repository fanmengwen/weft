...
describe('indexedDbStateStorage', () => {
  it('migrates localStorage value on first read', async () => {
    ...
    const value = await storage.getItem('weft-storage');

    expect(value).toBe('{"state":{"tabs":[]}}');
    expect(localStorageRef.getItem).toHaveBeenCalledWith('weft-storage');
  });
  ...
  it('writes and removes values through IndexedDB store', async () => {
    ...
    await storage.setItem('weft-storage', '{"state":{"activeTabId":"tab-2"}}');
    const storedValue = await storage.getItem('weft-storage');
    expect(storedValue).toBe('{"state":{"activeTabId":"tab-2"}}');

    await storage.removeItem('weft-storage');
    const afterDelete = await storage.getItem('weft-storage');
